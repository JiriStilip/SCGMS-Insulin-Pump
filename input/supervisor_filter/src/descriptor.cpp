...
namespace supervisor_filter {
	...
	const wchar_t* ui_param_name[param_count] = {
		L"CGM watchdog reset interval"
	};

	const wchar_t* rsWatchdogInterval = L"watchdog_interval";
	...
	const wchar_t* ui_param_tooltips[param_count] = {
		L"Interval after which the watchdog toggles specified action",
	};

	const wchar_t* filter_name = L"System Supervisor Filter";
...
const std::array<scgms::TFilter_Descriptor, 1> filter_descriptions = { { supervisor_filter::descriptor } };
...
extern "C" HRESULT IfaceCalling do_create_filter(const GUID *id, scgms::IFilter *output, scgms::IFilter **filter) {
	...
	if (*id == supervisor_filter::descriptor.id) {
		return Manufacture_Object<CSupervisor_Filter>(filter, output);
	}
